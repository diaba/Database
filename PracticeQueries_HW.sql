...
VALUES	 (1, 1, 1 )
		...
		,(5, 4, 4 )
-- --------------------------------------------------------------------------------
-- Query#  1)	Write a query that shows all golfers and their name 
--and email address only if they have a gmail email account. 
-- --------------------------------------------------------------------------------

SELECT (strFirstName +' '+strLastName) AS NAME, strEmail
FROM TGolfers
WHERE strEmail like '%gmail%'

-- --------------------------------------------------------------------------------
--2) Write a query to show all golfers sponsored from a particular sponsor for all events. 
--   Include sponsor names and golfer names. 
-- --------------------------------------------------------------------------------
SELECT        (TG.strLastName + ' ' + TG.strFirstName) AS GolferName, (TS.strFirstName + ' ' + TS.strLastName) AS SponsorName, TE.dtmEventDate
...
ORDER BY      TG.strFirstName, TG.strLastName

-- --------------------------------------------------------------------------------
-- 3)	Write a query showing the years a particular sponsor sponsored a golfer.   
-----------------------------------------------------------------------------------
SELECT        DISTINCT(TE.dtmEventDate),(TG.strFirstName + ' ' + TG.strLastName) AS GolferName, (TS.strFirstName + ' ' + TS.strLastName) AS SponsorName
FROM          TGolfers AS TG  JOIN TEventGolfers AS TEG 
			  ON TG.intGolferID = TEG.intGolferID 
...
WHERE         TS.intSponsorID = 1

-----------------------------------------------------------------------------------
--4) Write a query showing the total amount a particular sponsor sponsored golfers per year.
--If the sponsor did not sponsor for a particular year, it should show zero. 
-----------------------------------------------------------------------------------
SELECT        (TS.strFirstName+' '+ TS.strLastName) AS Name, SUM(TEGS.monPledgeAmount) AS TotalAmount, TE.intEventID
FROM          TEventGolfers AS TEG JOIN TEventGolferSponsors TEGS
			  ON TEG.intEventGolferID = TEGS.intEventGolferID 
			  JOIN TEvents AS TE
			  ...
			  ON TEGS.intSponsorID = TS.intSponsorID
--WHERE         TS.intSponsorID = 1
GROUP BY      TE.intEventID, TS.strFirstName, TS.strLastName
-----------------------------------------------------------------------------------
--5)	Write a query that list all sponsors who has sponsored a golfer in an event but not in the current event. 
-----------------------------------------------------------------------------------

-----------------------------------------------------------------------------------
--6)	Write a query to provide a list of all sponsors for a particular event.
--Show name of sponsor.  Ensure it never duplicates. 
-----------------------------------------------------------------------------------
SELECT        DISTINCT((TS.strFirstName + ' ' + TS.strLastName)) AS SponsorName, TE.dtmEventDate
FROM          TEventGolfers AS TEG  JOIN TEventGolferSponsors AS TEGS 
			  ON TEG.intEventGolferID = TEGS.intEventGolferID 
...
-----------------------------------------------------------------------------------
--7)	Write a query showing sponsors per state.  Show State and Sponsor Name
-----------------------------------------------------------------------------------
SELECT        TST.strStateDesc AS State, (TS.strFirstName + ' ' + TS.strLastName) AS SponsorName
FROM          TSponsors AS TS JOIN TStates AS TST
			  ON TST.intStateID = TS.intStateID  
-----------------------------------------------------------------------------------
--8)	Write a query to show the number of sponsors per state.   
--      Show zero if there aren’t any. 
-----------------------------------------------------------------------------------
SELECT        TST.strStateDesc AS State, COUNT( TS.intSponsorID) AS Sponsor
FROM          TSponsors AS TS RIGHT JOIN TStates AS TST
			  ON TST.intStateID = TS.intStateID  
GROUP BY      TST.strStateDesc
-----------------------------------------------------------------------------------
--9)	Write a query showing sponsors that only sponsored female golfers. 
-----------------------------------------------------------------------------------
SELECT        (TG.strLastName + ' ' + TG.strFirstName) AS GolferName, (TS.strFirstName + ' ' + TS.strLastName) AS SponsorName
FROM          TGolfers AS TG  JOIN TEventGolfers AS TEG 
			  ON TG.intGolferID = TEG.intGolferID 

			  JOIN TGenders AS TGe 
			  ON TG.intGolferID = TGe.intGenderID
...
WHERE         TGe.strGenderDesc = 'Female'

-----------------------------------------------------------------------------------
--10)	Write a query showing all sponsors who last name begins with “G”. 
 -----------------------------------------------------------------------------------
SELECT        (TS.strFirstName + ' ' + TS.strLastName) AS SponsorName
FROM          TSponsors AS TS 			  
WHERE         TS.strLastName LIKE 'G%'
