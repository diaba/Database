...
-- --------------------------------------------------------------------------------
-- 1
SELECT TC.strFirstName,TC.strLastName, TOS.strOrderNumber, TOS.dtmOrderDate , TS.strStatus
FROM    TCustomers		as TC
	   ,TOrders		    as TOS
	   ,TStatus         as TS
Where	TC.intCustomerID	= TOS.intCustomerID
   and  TOS.intStatusID = TS.intStatusID
Order By dtmOrderDate DESC

--2
SELECT TC.strFirstName,TC.strLastName, TG.strGender, TOS.strOrderNumber, TOS.dtmOrderDate 
FROM    TCustomers		as TC
	   ,TOrders		    as TOS
	   ,TGenders        as TG
Where	TC.intCustomerID	= TOS.intCustomerID
   and  TC.intGenderID = TG.intGenderID
   and  TOS.dtmOrderDate > '2015-12-12'
Order By dtmOrderDate DESC ,TG.strGender

--3
SELECT TC.strFirstName,TC.strLastName, TG.strGender,TR.strRace, TOS.strOrderNumber, TOS.dtmOrderDate 
FROM    TCustomers		as TC
	   ,TOrders		    as TOS
	   ,TGenders        as TG
	   ,TRaces          as TR
Where	TC.intCustomerID	= TOS.intCustomerID
   and  TG.intGenderID = TC.intGenderID
   and  TC.intRaceID = TR.intRaceID
   and  TOS.dtmOrderDate < '2017-1-1'
Order By dtmOrderDate DESC ,TR.strRace,TG.strGender

--4
SELECT TC.strFirstName,TC.strLastName, TS.strStatus,TR.strRace, TOS.strOrderNumber, TOS.dtmOrderDate , TST.strState
FROM    TCustomers		as TC
	   ,TOrders		    as TOS
	   ,TStatus         as TS
	   ,TRaces          as TR
	   ,TStates         as TST
Where	TC.intCustomerID	= TOS.intCustomerID
   and  TS.intStatusID = TOS.intStatusID
   and  TC.intRaceID = TR.intRaceID
   and  TC.intStateID = TST.intStateID
Order By  TST.strState, dtmOrderDate
