...
-- --------------------------------------------------------------------------------
-- Aggregate #2  -- Show the total count of golfer sponsors per year. 
-- --------------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------------
-- Aggregate #3 Show the total amount pledged/donated per year from golfer sponsors.
-- --------------------------------------------------------------------------------
SELECT	TE.dtmEventDate 
...
-- --------------------------------------------------------------------------------
-- Aggregate #4  Show total amount collected for a particular year from golfer sponsors.
-- --------------------------------------------------------------------------------
SELECT	TE.dtmEventDate 
...
-- --------------------------------------------------------------------------------
-- Aggregate #5 Show all golfers who collected over $2500 for a 
-- particular year from golfer sponsors.
-- --------------------------------------------------------------------------------
SELECT	TE.dtmEventDate, TG.strFirstName, TG.strLastName
...
GROUP BY TE.dtmEventDate, TG.strFirstName, TG.strLastName
HAVING SUM(TEGS.monPledgeAmount * 100) > 2500

----------------------------------------------------------------------------------
-- Aggregate #6 Show the total pledges per golfer for a particular year.  
...
Group By TE.dtmEventDate
Order by SUM(TEGS.monPledgeAmount * 100) DESC
-- --------------------------------------------------------------------------------
-- Aggregate #7 Show the average pledge of all sponsors per year. 
-- --------------------------------------------------------------------------------
SELECT	   TE.dtmEventDate 
	      ,AVG(TEGS.monPledgeAmount * 100) AS TotalPledged
...
-- 8)	Show the golfer sponsor who gave the highest single pledge. 

SELECT      MAX(TEGS.monPledgeAmount * 100) AS [Highest Pledge], ( TS.strFirstName + ' ' +TS.strLastName) AS Name

FROM        TEventGolferSponsors AS TEGS INNER JOIN TEventGolfers AS TEG
            ON TEG.intEventGolferID = TEGS.intEventGolferID

			INNER JOIN TSponsors AS TS
			ON TS.intSponsorID = TEGS.intSponsorID

			INNER JOIN TGolfers AS TG
			ON TG.intGolferID = TEG.intGolferID

GROUP BY    TS.strFirstName, TS.strLastName

ORDER BY    TS.strLastName

-- 9)	Show the golfer sponsor who gave the lowest single pledge. 

SELECT      MIN(TEGS.monPledgeAmount * 100) AS [Highest Pledge], ( TS.strFirstName + ' ' +TS.strLastName) AS Name
...
SELECT      MAX(TEGS.monPledgeAmount * 100) AS MaxPledge , 
                   TE.dtmEventDate , TTT.strTypeofTeamDesc , TL.strLevelDesc

FROM               TTeamandClubs AS TTC INNER JOIN TEventGolferTeamandClubs AS TEGTC
                   ON  TTC.intTeamandClubID = TEGTC.intTeamandClubID

				   INNER JOIN TTypeofTeams AS TTT
				   ON TTT.intTypeofTeamID = TTC.intTypeofTeamID

				   INNER JOIN TLevelofTeams AS TL
				   ON TL.intLevelofTeamID = TTC.intLevelofTeamID

				   INNER JOIN TEventGolfers AS TEG 
				   ON TEG.intEventGolferID = TEGTC.intEventGolferID

				   INNER JOIN TEventGolferSponsors AS TEGS
				   ON TEGS.intEventGolferID = TEG.intEventGolferID

				   INNER JOIN TEvents AS TE
				   ON TE.intEventID = TEG.intEventID

WHERE              TE.intEventID = 1

GROUP BY          TE.dtmEventDate , TTT.strTypeofTeamDesc, TL.strLevelDesc

ORDER BY          TE.dtmEventDate , TTT.strTypeofTeamDesc, TL.strLevelDesc

-- 11)	Show the total amount raised in corporate sponsorships for a particular year. 
SELECT	    TE.dtmEventDate 
	        ,SUM(TEGS.monPledgeAmount * 100) AS TotalRaised
